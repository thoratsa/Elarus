...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background: #0f0f0f; color: #e0e0e0; line-height: 1.6; min-height: 100vh; }
        .container { min-height: 100vh; display: flex; flex-direction: column; background: #1a1a1a; }
        .header { background: #000; color: white; padding: 2rem; text-align: center; border-bottom: 1px solid #333; }
        .header h1 { font-size: 2.5rem; margin-bottom: 0.5rem; font-weight: 600; background: linear-gradient(135deg, #667eea, #764ba2); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
        .subtitle { font-size: 1.3rem; margin-bottom: 0.5rem; opacity: 0.9; }
        .tagline { opacity: 0.7; font-size: 1rem; }
        ...
        label { display: block; margin-bottom: 0.5rem; font-weight: 500; color: #ccc; font-size: 0.9rem; }
        textarea, input { width: 100%; padding: 1rem; border: 1px solid #333; border-radius: 8px; font-size: 1rem; font-family: inherit; background: #2d2d2d; color: #e0e0e0; transition: all 0.2s ease; }
        textarea:focus, input:focus { outline: none; border-color: #667eea; box-shadow: 0 0 0 2px rgba(102, 126, 234, 0.1); }
        textarea { height: 140px; resize: vertical; line-height: 1.5; }
        .char-counter { text-align: right; font-size: 0.8rem; color: #888; margin-top: 0.25rem; }
        .button-group { display: flex; gap: 1rem; margin-bottom: 2rem; }
        .btn { flex: 1; padding: 1rem 1.5rem; border: none; border-radius: 8px; font-size: 1rem; font-weight: 500; cursor: pointer; transition: all 0.2s ease; font-family: inherit; }
        .btn-primary { background: #667eea; color: white; }
        .btn-primary:hover:not(:disabled) { background: #5a6fd8; transform: translateY(-1px); }
        .btn-secondary { background: #555; color: white; }
        .btn-secondary:hover:not(:disabled) { background: #666; transform: translateY(-1px); }
        .btn:disabled { opacity: 0.5; cursor: not-allowed; transform: none; }
        .result { background: #2d2d2d; border-radius: 8px; padding: 1.5rem; margin-top: 1rem; border: 1px solid #333; font-family: monospace; font-size: 0.9rem; }
        .result h3 { color: #fff; margin-bottom: 1rem; font-size: 1.1rem; }
        .result-content { white-space: pre-wrap; line-height: 1.5; color: #e0e0e0; }
        .status { display: inline-block; padding: 0.25rem 0.75rem; border-radius: 12px; font-size: 0.75rem; font-weight: 600; margin-bottom: 1rem; }
        .status-cached { background: #1e3a28; color: #4ade80; border: 1px solid #2d4a3a; }
        .status-generated { background: #1e3a5f; color: #60a5fa; border: 1px solid #2d4a7a; }
        .status-regenerated { background: #5c4a1e; color: #fbbf24; border: 1px solid #7a5a2d; }
        .error { background: #3a1e1e; border-color: #7a2d2d; }
        .loading { text-align: center; padding: 2rem; color: #888; font-style: italic; }
        .api-info { background: #2d2d2d; border-radius: 8px; padding: 1.5rem; margin-top: 2rem; border: 1px solid #333; grid-column: 1 / -1; }
        .api-info h3 { color: #fff; margin-bottom: 1rem; font-size: 1.1rem; }
        .api-info p { margin-bottom: 0.5rem; font-size: 0.9rem; color: #ccc; }
        .api-info code { background: #000; color: #667eea; padding: 0.2rem 0.5rem; border-radius: 4px; font-family: monospace; font-size: 0.8rem; }
        .json-key { color: #f78c6c; font-weight: 600; }
        .json-string { color: #89ddff; }
        .json-number { color: #f78c6c; }
        .json-boolean { color: #c792ea; }
        @media (max-width: 767px) { .content { padding: 1rem; } .header { padding: 1.5rem 1rem; } .header h1 { font-size: 2rem; } .content-grid { grid-template-columns: 1fr; } .button-group { flex-direction: column; } }
    </style>
</head>
<body>
    <div class="container">
            ...
            <div class="content-grid">
                <div class="input-section">
                    ...
                    <div class="button-group">
                        <button class="btn btn-primary" onclick="translateText()" id="translateBtn">Translate</button>
                        <button class="btn btn-secondary" onclick="retranslateText()" id="retranslateBtn">Retranslate</button>
                    </div>
                </div>
    ...
    <script>
        ...
        function showResult(content, isError = false) {
            ...
            if (isError) { resultDiv.classList.add('error'); } else { resultDiv.classList.remove('error'); }
        }
        async function makeRequest(endpoint) {
            if (currentRequest) { currentRequest.abort(); }
            const text = document.getElementById('text').value.trim();
            const targetLang = document.getElementById('targetLang').value.trim();
            if (!text) { showResult('<div class="error"><h3>Error</h3><div class="result-content">Please enter text to translate</div></div>', true); return; }
            if (!targetLang) { showResult('<div class="error"><h3>Error</h3><div class="result-content">Please enter target language</div></div>', true); return; }
            if (text.length > 2000) { showResult('<div class="error"><h3>Error</h3><div class="result-content">Text exceeds maximum length of 2000 characters</div></div>', true); return; }
            setLoading(true);
            ...
            try {
                ...
                showResult(`<div class="result"><span class="status ${statusClass}">${statusText}</span><h3>API Response</h3><div class="result-content">${formattedJSON}</div></div>`);
            } catch (error) {
                if (error.name === 'AbortError') { return; }
                showResult(`<div class="result error"><h3>Error</h3><div class="result-content">${error.message}${error.details ? '<br><br><strong>Details:</strong> ' + error.details : ''}</div></div>`, true);
            } finally {
                setLoading(false);
                currentRequest = null;
            }
        }
        function translateText() { makeRequest('/api/translate'); }
        function retranslateText() { makeRequest('/api/retranslate'); }
        document.getElementById('targetLang').addEventListener('keypress', function(e) { if (e.key === 'Enter') { translateText(); } });
    </script>
</body>
</html>
'''
